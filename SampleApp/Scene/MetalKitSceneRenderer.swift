...
class MetalKitSceneRenderer: NSObject, MTKViewDelegate {
    ...
    private func preSortAllFrames() async {
        ...
        // Calculate initial camera position from current viewport
        // Use default viewport if drawableSize is not yet set
        let initialViewport: ModelRendererViewportDescriptor
        if drawableSize.width > 0 && drawableSize.height > 0 {
            initialViewport = viewport
        } else {
            // Fallback to default viewport if size not available yet
            ...
            initialViewport = ModelRendererViewportDescriptor(
                viewport: MTLViewport(originX: 0, originY: 0, width: 1024, height: 1024, znear: 0, zfar: 1),
                projectionMatrix: projectionMatrix,
                viewMatrix: translationMatrix * commonUpCalibration,
                screenSize: SIMD2(x: 1024, y: 1024)
            )
        }
        
        let initialCameraPosition = SplatRenderer.cameraWorldPosition(forViewMatrix: initialViewport.viewMatrix)
        let initialCameraForward = SplatRenderer.cameraWorldForward(forViewMatrix: initialViewport.viewMatrix)
    ...
    private func batchSortUpcomingFrames() {
        ...
        // Get current camera position from viewport
        let currentViewport = viewport
        let currentCameraPosition = SplatRenderer.cameraWorldPosition(forViewMatrix: currentViewport.viewMatrix)
        let currentCameraForward = SplatRenderer.cameraWorldForward(forViewMatrix: currentViewport.viewMatrix)
        ...
        Task.detached(priority: .userInitiated) { [weak self] in
            ...
            for offset in 1...framesToSort {
                ...
                // Check if camera has moved significantly (same threshold as in SplatRenderer: 0.01)
